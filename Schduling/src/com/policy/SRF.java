...
public class SRF {
	...
	public void simulate() {
		...
		while (remainingProcess > 0)
		{
			...
			for (int i = 0; i < count; i++) 
			{
				...
				if (current.getRemainingTime() > 0 && current.getRemainingTime()<min &&current.getArrivalTime()<=currentTime )
				{	
					index=i;
					min=current.getRemainingTime();
				}	
			}
			if(index==-1)
			{	currentTime++;
	...
	public void printResult() {
		...
		System.out
				.println("Simulation result of SRF Schduling ");
		System.out
		...
		System.out.println("Average Waiting Time of SRF "
				+ avgWatingTime);
		System.out.println("Average TurnAround Time of SRF "
				+ avgTurnaroundTime);
		for(int i=0;i<j;i++)
		{
